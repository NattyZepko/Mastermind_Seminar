...
# def runSecondWindow(player_type, game_count = 1, black_included=0, num_of_colors=4):
#     top = Toplevel()
...
gameCount = IntVar()
gameCount.set(1)
gameCountSpinBox = Spinbox(root, from_=1, to=99999, textvariable=gameCount, width=5)

num_of_colors = IntVar()
num_of_colors.set(4)
numOfColorsSpinBox = Spinbox(root, from_=4, textvariable=num_of_colors, to=10, width=3)

numToGuessLabel = Label(root, text="Number of Colors to guess")
startButton = Button(root, text="START!", command=lambda: gw.StartGame("Player", gameCount.get(), CheckVar.get(), num_of_colors.get()))
...
includeBlackCheckBox = Checkbutton(root, text="Include the color black", variable=CheckVar)
...
includeBlackCheckBox.grid(row=3, column=0, columnspan=10)
numOfColorsSpinBox.grid(row=4, column=0)
