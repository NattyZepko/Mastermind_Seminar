...
class Game_AI:
     ...
     def populateAllRows(self, rowPosition, guess_index):
        SpinBoxList = []
        ...
        for column_idx in range(self.numOfDigits):  
            ...
            SpinBoxList[column_idx].grid(row=rowPosition, column=column_idx)
        ...
        guess_result_label.grid(row=rowPosition, column=self.numOfDigits)
        if (guess_index<len(self.allGuesses)-1):
            guess_result_label.after(self.sleeping_Time, lambda: self.populateAllRows(rowPosition+1, guess_index+1))
